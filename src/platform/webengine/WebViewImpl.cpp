...
WebViewImpl::WebViewImpl(std::unique_ptr<BlinkWebView> webview)
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: script:%s",
        __func__, script.c_str());
    if (m_webView)
        m_webView->addUserScript(script);
}
...
void WebViewImpl::setDelegate(WebPageBlinkDelegate* delegate)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    if (m_webView)
        m_webView->setDelegate(delegate);
}

WebPageBlinkDelegate* WebViewImpl::delegate()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->delegate() : nullptr;
}

int WebViewImpl::progress()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->progress() : 0;
}
    ...
    bool use_native_scroll)
{
    LOG_INFO(
        MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: app_id:\"%s\" app_path:\"%s\" trust_level:\"%s\" "
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->DefaultUserAgent() : "";
}

std::string WebViewImpl::UserAgent() const
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->UserAgent() : "";
}

void WebViewImpl::LoadUrl(const std::string& url)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: url:\"%s\"",
        __func__, url.c_str());
    if (m_webView)
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: name:\"%s\"",
        __func__, name.c_str());
    if (m_webView)
...
void WebViewImpl::EnableInspectablePage()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    if (m_webView)
        m_webView->EnableInspectablePage();
}
...
void WebViewImpl::DisableInspectablePage()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    if (m_webView)
        m_webView->DisableInspectablePage();
}
...
void WebViewImpl::AddAvailablePluginDir(const std::string& directory)
{
    ...
    if (m_webView)
        m_webView->AddAvailablePluginDir(directory);
}
...
void WebViewImpl::SetUserAgent(const std::string& useragent)
{
    ...
    if (m_webView)
        m_webView->SetUserAgent(useragent);
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: suppress:\"%d\"", __func__, suppress);
    if (m_webView)
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: enabled:\"%d\"", __func__, enabled);
    if (m_webView)
        m_webView->SetUseAccessibility(enabled);
}

void WebViewImpl::SetActiveOnNonBlankPaint(bool active)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: active:\"%d\"", __func__, active);
    if (m_webView)
        m_webView->SetActiveOnNonBlankPaint(active);
}

void WebViewImpl::SetViewportSize(int width, int height)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: width:\"%d\" height:\"%d\"",
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: level:\"%d\"", __func__, level);
    if (m_webView)
        m_webView->NotifyMemoryPressure(level);
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: visibilityState:\"%d\"",
        __func__, visibilityState);
    if (m_webView)
        m_webView->SetVisibilityState(visibilityState);
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->DocumentTitle() : "";
}
...
void WebViewImpl::RunJavaScript(const std::string& js_code)
{
    ...
    if (m_webView)
        m_webView->RunJavaScript(js_code);
}
...
void WebViewImpl::RunJavaScriptInAllFrames(const std::string& js_code)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: js_code:\"%s\"",
        __func__, js_code.c_str());
    if (m_webView)
        m_webView->RunJavaScriptInAllFrames(js_code);
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->RenderProcessPid() : 0;
}
...
bool WebViewImpl::IsDrmEncrypted(const std::string& url)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: url:\"%s\"",
        __func__, url.c_str());
    return m_webView ? m_webView->IsDrmEncrypted(url) : false;
}
...
std::string WebViewImpl::DecryptDrm(const std::string& url)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: url:\"%s\"",
        __func__, url.c_str());
    return m_webView ? m_webView->DecryptDrm(url) : "";
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->GetZoomFactor() : 0;
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: dnt:\"%d\"",
        __func__, dnt);
    if (m_webView)
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->CanGoBack() : false;
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->IsInputMethodActive() : false;
}

void WebViewImpl::SetAdditionalContentsScale(float scale_x, float scale_y)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: scale_x:\"%f\" scale_x:\"%f\"",
        __func__, scale_x, scale_y);
    if (m_webView)
        m_webView->SetAdditionalContentsScale(scale_x, scale_y);
}
...
void WebViewImpl::SetEnableHtmlSystemKeyboardAttr(bool enabled)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enabled:\"%d\"",
        __func__, enabled);
    if (m_webView)
        m_webView->SetEnableHtmlSystemKeyboardAttr(enabled);
}

void WebViewImpl::RequestInjectionLoading(const std::string& injection_name)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: injection_name:\"%s\"", __func__,
        injection_name.c_str());
    if (m_webView)
        m_webView->RequestInjectionLoading(injection_name);
}
...
void WebViewImpl::ActivateRendererCompositor()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    if (m_webView)
        m_webView->ActivateRendererCompositor();
}
...
void WebViewImpl::DeactivateRendererCompositor()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    if (m_webView)
        m_webView->DeactivateRendererCompositor();
}
...
void WebViewImpl::ResetStateToMarkNextPaint()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    if (m_webView)
        m_webView->ResetStateToMarkNextPaint();
}
...
void WebViewImpl::SetAllowRunningInsecureContent(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetAllowRunningInsecureContent(enable);
}
...
void WebViewImpl::SetAllowScriptsToCloseWindows(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetAllowScriptsToCloseWindows(enable);
}
...
void WebViewImpl::SetAllowUniversalAccessFromFileUrls(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetAllowUniversalAccessFromFileUrls(enable);
}
...
void WebViewImpl::SetRequestQuotaEnabled(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetRequestQuotaEnabled(enable);
}
...
void WebViewImpl::SetSuppressesIncrementalRendering(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetSuppressesIncrementalRendering(enable);
}
...
void WebViewImpl::SetDisallowScrollbarsInMainFrame(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetDisallowScrollbarsInMainFrame(enable);
}
...
void WebViewImpl::SetDisallowScrollingInMainFrame(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetDisallowScrollingInMainFrame(enable);
}
...
void WebViewImpl::SetJavascriptCanOpenWindows(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetJavascriptCanOpenWindows(enable);
}
...
void WebViewImpl::SetSpatialNavigationEnabled(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetSpatialNavigationEnabled(enable);
}
...
void WebViewImpl::SetSupportsMultipleWindows(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetSupportsMultipleWindows(enable);
}
...
void WebViewImpl::SetCSSNavigationEnabled(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetCSSNavigationEnabled(enable);
}

void WebViewImpl::SetV8DateUseSystemLocaloffset(bool use)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: use:\"%d\"",
        __func__, use);
    if (m_webView)
        m_webView->SetV8DateUseSystemLocaloffset(use);
}
...
void WebViewImpl::SetAllowLocalResourceLoad(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetAllowLocalResourceLoad(enable);
}
...
void WebViewImpl::SetLocalStorageEnabled(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetLocalStorageEnabled(enable);
}
...
void WebViewImpl::SetWebSecurityEnabled(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetWebSecurityEnabled(enable);
}
...
void WebViewImpl::SetKeepAliveWebApp(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetKeepAliveWebApp(enable);
}
...
void WebViewImpl::SetAdditionalFontFamilyEnabled(bool enable)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%d\"",
        __func__, enable);
    if (m_webView)
        m_webView->SetAdditionalFontFamilyEnabled(enable);
}
...
void WebViewImpl::SetNetworkStableTimeout(double timeout)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enable:\"%f\"",
        __func__, timeout);
    if (m_webView)
        m_webView->SetNetworkStableTimeout(timeout);
}
...
void WebViewImpl::SetAllowFakeBoldText(bool allow)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: allow:\"%d\"",
        __func__, allow);
    if (m_webView)
        m_webView->SetAllowFakeBoldText(allow);
}
...
void WebViewImpl::SetSecurityOrigin(const std::string& identifier)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: identifier:\"%s\"",
        __func__, identifier.c_str());
    if (m_webView)
        m_webView->SetSecurityOrigin(identifier);
}
...
void WebViewImpl::SetAcceptLanguages(const std::string& lauguages)
{
    ...
    if (m_webView)
        m_webView->SetAcceptLanguages(lauguages);
}
...
void WebViewImpl::SetBoardType(const std::string& board_type)
{
    ...
    if (m_webView)
        m_webView->SetBoardType(board_type);
}
...
void WebViewImpl::SetMediaPreferences(const std::string& preferences)
{
    ...
    if (m_webView)
        m_webView->SetMediaPreferences(preferences);
}
...
void WebViewImpl::SetSearchKeywordForCustomPlayer(bool enabled)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enabled:\"%d\"",
        __func__, enabled);
    if (m_webView)
        m_webView->SetSearchKeywordForCustomPlayer(enabled);
}
...
void WebViewImpl::SetUseUnlimitedMediaPolicy(bool enabled)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enabled:\"%d\"",
        __func__, enabled);
    if (m_webView)
        m_webView->SetUseUnlimitedMediaPolicy(enabled);
}

void WebViewImpl::SetAudioGuidanceOn(bool on)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: on:\"%d\"",
        __func__, on);
    if (m_webView)
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: font:\"%s\"",
        __func__, font.c_str());
    if (m_webView)
        m_webView->SetStandardFontFamily(font);
}
...
void WebViewImpl::SetFixedFontFamily(const std::string& font)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: font:\"%s\"",
        __func__, font.c_str());
    if (m_webView)
        m_webView->SetFixedFontFamily(font);
}
...
void WebViewImpl::SetSerifFontFamily(const std::string& font)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: font:\"%s\"",
        __func__, font.c_str());
    if (m_webView)
        m_webView->SetSerifFontFamily(font);
}
...
void WebViewImpl::SetSansSerifFontFamily(const std::string& font)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: font:\"%s\"",
        __func__, font.c_str());
    if (m_webView)
        m_webView->SetSansSerifFontFamily(font);
}
...
void WebViewImpl::SetCursiveFontFamily(const std::string& font)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: font:\"%s\"",
        __func__, font.c_str());
    if (m_webView)
        m_webView->SetCursiveFontFamily(font);
}
...
void WebViewImpl::SetFantasyFontFamily(const std::string& font)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: font:\"%s\"",
        __func__, font.c_str());
    if (m_webView)
        m_webView->SetFantasyFontFamily(font);
}
...
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: url:\"%s\" font:\"%s\"",
        __func__, url.c_str(), font.c_str());
    if (m_webView)
        m_webView->LoadAdditionalFont(url, font);
}

void WebViewImpl::SetUseLaunchOptimization(bool enabled, int delay_ms)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0,
        ">>>>>>> WebViewImpl::%s: enabled:\"%d\" delay_ms:\"%d\"",
        __func__, enabled, delay_ms);
    if (m_webView)
        m_webView->SetUseLaunchOptimization(enabled, delay_ms);
}
...
void WebViewImpl::SetAppPreloadHint(bool is_preload)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: is_preload:\"%d\"",
        __func__, is_preload);
    if (m_webView)
        m_webView->SetAppPreloadHint(is_preload);
}
...
void WebViewImpl::SetTransparentBackground(bool enabled)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: enabled:\"%d\"",
        __func__, enabled);
    if (m_webView)
        m_webView->SetTransparentBackground(enabled);
}
...
void WebViewImpl::SetBackHistoryAPIDisabled(const bool on)
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s: on:\"%d\"",
        __func__, on);
    if (m_webView)
        m_webView->SetBackHistoryAPIDisabled(on);
}

content::WebContents* WebViewImpl::GetWebContents()
{
    LOG_INFO(MSGID_WAM_DEBUG, 0, ">>>>>>> WebViewImpl::%s", __func__);
    return m_webView ? m_webView->GetWebContents() : nullptr;
}
